    ...
    if (fields === "all") {
        blog = {
            ...
            category: {
                ...
                path: `/articles/category/${slugify(blogData.category)}`,
            },
            subcategory: blogData?.subcategory,
            tags: blogData.tags.map((tag) => ({
                title: tag,
                slug: slugify(tag),
                path: `/articles/tag/${slugify(tag)}`,
            })),
            slug: realSlug,
        ...
        blog = fields.reduce((acc: IBlog, field: keyof IBlog) => {
            ...
            if (field === "category") {
                return {
                    ...
                    category: {
                        title: blogData.category,
                        slug: slugify(blogData.category),
                        path: `/articles/category/${slugify(
                            blogData.category
                        )}`,
                    },
                };
            ...
            if (field === "subcategory") {
                return {
                    ...acc,
                    subcategory: {
                        title: blogData.subcategory,
                        slug: slugify(blogData.subcategory),
                        path: `/articles/category/${slugify(
                            blogData.category
                        )}/${slugify(blogData.subcategory)}/`,
                        subcategory: blogData?.subcategory,
                    },
                };
            }
...
export function getAllBlogs(
    ...
    let blogs = slugs.map((slug) => getPostBySlug(slug, fields));
...
export function getPostsBySubCategory(
    ...
    const postFields =
        fields === "all"
            ? "all"
            : ([...fields, "subcategory"] as Array<keyof IBlog>);
    const { blogs } = getAllBlogs(postFields);
    let result = blogs.filter((post) => post.subcategory === subcategory);
    const totalPosts = result.length;
    if (limit) result = result.slice(skip, skip + limit);
    return { posts: result, count: totalPosts };
}
